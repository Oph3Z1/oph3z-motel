...
RegisterCommand("oxskm", function ()
    exports.ox_inventory:openInventory('stash', {id='42wallabyway', owner=123})
    -- TriggerServerEvent('ox:loadStashes')
    -- exports.ox_inventory:openInventory('stash',  11)
end)
...
function OpenBossMenu(id)
    ...
    ESX.TriggerServerCallback("oph3z-motels:server:RequestMotel", function(RequestData)
        if distance ~= -1 and distance <= 3.0 then
            ESX.TriggerServerCallback("oph3z-motels:server:PlayerName", function(name)
                local data = {
                    motel = Config.Motels[id],
                    rooms = Config.Motels[id].Rooms,
...
function UseTargetClient(coords, eventname, eventneresi, eventlabel)
    ...
    if not motelZones[eventneresi] then
        exports["qb-target"]:AddBoxZone("oph3z-motels:UseTarget:OpenMotel", vector3(coords.x, coords.y, coords.z), 1, 2.4, {
            name = eventneresi,
            heading=270,
            minZ= coords.z-5,
            maxZ= coords.z +5
...
RegisterNetEvent("oph3z-motels:client:UseTarget", function (eventneresi)
                ...
                if v.Owner == "" then
                    if #(PlayerCoords - v.RentMotel) <= 2.0 then
                        ...
                        if Config.Data.UseTarget then
                            targetmotelid = k
                            UseTargetClient(v.RentMotel, "oph3z-motels:client:UseTarget", "RentMotel", "Rent Motel")
                        else
                            DrawText3D("~INPUT_PICKUP~ - Open Motel", vector3(v.RentMotel.x, v.RentMotel.y, v.RentMotel.z))
            ...
            for k,v in pairs(Config.Motels) do
                ...
                if v.Employes ~= nil then
                    for key, value in pairs(v.Employes) do
                        employesExist = true 
                        if v.Owner == tostring(ESX.PlayerData.identifier) or value.Citizenid == tostring(ESX.PlayerData.identifier)  then
                            if #(PlayerCoords - v.OpenBossMenu) <= 2.0 then
                                sleep = 0
                                if Config.Data.UseTarget then
                                    targetboosmenuid = k
                                    UseTargetClient(v.OpenBossMenu, "oph3z-motels:client:UseTarget", "OpenBossMenu", "Open Boss Menu")
                                else 
                                    DrawText3D("~INPUT_PICKUP~ - Open Boss Menu", vector3(v.OpenBossMenu.x, v.OpenBossMenu.y, v.OpenBossMenu.z))
                                    if IsControlJustReleased(0, 38) then
                                        OpenBossMenu(k)
                                    end
                                end
                ...
                if not employesExist then
                    if v.Owner == tostring(ESX.PlayerData.identifier) then
                        if #(PlayerCoords - v.OpenBossMenu) <= 2.0 then
                            sleep = 0
                            if Config.Data.UseTarget then
                                targetboosmenuid = k
...
function AcceptMotelRoom(data)
    ESX.TriggerServerCallback("oph3z-motels:server:RentRoom", function (export)
        if export then
...
CreateThread(function ()
    while true do 
        Citizenid = tostring(ESX.PlayerData.identifier)
        local sleep = 1000
        local Player = PlayerPedId()
        local PlayerCoords = GetEntityCoords(Player)
        for k,v in pairs (Config.Motels) do
                                        ...
                                        if room.Owner.RoomsOwner == Citizenid or v.Owner == Citizenid or employees.Citizenid == Citizenid or friendscitizenid == Citizenid then
                                            if #(PlayerCoords - vector3(v.Rooms[i].Coords.x, v.Rooms[i].Coords.y, v.Rooms[i].Coords.z)) <= 2.0 then
                                                ...
                                                if Config.Data.UseTarget then
                                                    ...
                                                    UseTargetClient(vector3(v.Rooms[i].Coords.x, v.Rooms[i].Coords.y, v.Rooms[i].Coords.z), "oph3z-motels:client:UseTarget", "OpenRoom", "Open Room")
                                                else
                                                    if IsControlJustReleased(0, 38) then
                                                        DrawText3D("~INPUT_PICKUP~ - Open Room", vector3(v.Rooms[i].Coords.x, v.Rooms[i].Coords.y, v.Rooms[i].Coords.z))
                                                        if employees.Citizenid == Citizenid and room.Owner.RoomsOwner ~= Citizenid then
                                                            OwnerMode = true
                                                        end
                                                        if v.Owner == Citizenid and room.Owner.RoomsOwner ~= Citizenid and friendscitizenid ~= Citizenid then
        ...
        if motelodasinda then
            ...
            for k, v in pairs(Config.Maps) do
                ...
                if Exit <= 2.0 then
                    sleep = 0
                    if Config.Data.UseTarget then
                        UseTargetClient(vector3(v.out.x, v.out.y, v.out.z), "oph3z-motels:client:UseTarget", "ExitRoom", "Exit")
                    else
                        DrawText3D("~INPUT_PICKUP~ - Exit", vector3(v.out.x, v.out.y, v.out.z ))
                        if IsControlJustReleased(0, 38) then
                            TriggerServerEvent("oph3z-motels:server:ExitRoom", Motelid, Odano)
                        end
                    end
                ...
                if Stash <= 2.0 then
                    if not OwnerMode then
                        sleep = 0
                        if Config.Data.UseTarget then
                            UseTargetClient(vector3(v.stash.x, v.stash.y, v.stash.z), "oph3z-motels:client:UseTarget", "Stash", "Stash")
                        else
                            DrawText3D("~INPUT_PICKUP~ - Stash", vector3(v.stash.x, v.stash.y, v.stash.z))
                            if IsControlJustReleased(0, 38) then
                                Config.StashFunction(Motelid, Odano, OdaType)
                            end
                        end
                ...
                if Config.Data.CustomersManage then
                    if Manage <= 2.0 then
                        if not OwnerMode then
                            sleep = 0
                            if Config.Data.UseTarget then
                                UseTargetClient(vector3(v.manage.x, v.manage.y, v.manage.z), "oph3z-motels:client:UseTarget", "Manage", "Manage")
                            else
                                DrawText3D("~INPUT_PICKUP~ - Manage", vector3(v.manage.x, v.manage.y, v.manage.z))
                                if IsControlJustReleased(0, 38) then
                                    ManageFunction(Motelid, Odano, OdaType, odatipi, OdaTheme)
                                end
                            end
                ...
                if Wardrobe <= 2.0 then
                    if not OwnerMode then
                        sleep = 0
                        if Config.Data.UseTarget then
                            UseTargetClient(vector3(v.wardrobe.x, v.wardrobe.y, v.wardrobe.z), "oph3z-motels:client:UseTarget", "Wardrobe", "Wardrobe")
                        else
                            DrawText3D("~INPUT_PICKUP~ - Wardrobe", vector3(v.wardrobe.x, v.wardrobe.y, v.wardrobe.z))
                            if IsControlJustReleased(0, 38) then
                                Config.WardrobeFunction()
                            end
                        end
        ...
        if Config.Data.BuyMotelPass then
            if #(PlayerCoords - vector3(Config.Data.BuyMotel.x, Config.Data.BuyMotel.y, Config.Data.BuyMotel.z)) <= 2.0 then
                ...
                if Config.Data.UseTarget then
                    ...
                    UseTargetClient(vector3(Config.Data.BuyMotel.x, Config.Data.BuyMotel.y, Config.Data.BuyMotel.z), "oph3z-motels:client:UseTarget", "BuyMotel", "Buy Motel")
                else
                    DrawText3D("~INPUT_PICKUP~ - Buy Motel", vector3(Config.Data.BuyMotel.x, Config.Data.BuyMotel.y, Config.Data.BuyMotel.z))
                    if IsControlJustReleased(0, 38) then
                        buymotelcoords = GetEntityCoords(PlayerPedId())
                        OpenBuyMenu()
                    end
...
RegisterCommand("buneab", function ()
...
function NerabyPlayers(data)
    ...
    elseif data.managementDoor then
        if data.Coords ~= nil then
            ...
            ESX.TriggerServerCallback("oph3z-motels:server:RoomInvite", function(name)
                SendNUIMessage({
                    type = "LoadPlayers1",
                    RoomInvite = name,
                })
